...
function defaultExtractTextToIdSuffix(text) {
  return trimSentence(text.replaceAll(" ", "_").replace(/[\W]+/g,""), 20).toLowerCase()
}

function defaultAttributeIdGenerator(context, node, text, idPrefix, extractTextToIdSuffix) {
  let id = context.getFilename() ? generateId(context, idPrefix, text) : "";
  id = id + "." + node.parent.name.name + "." + extractTextToIdSuffix(text);
  return id;
}

function defaultLiteralIdGenerator(context, text, idPrefix, extractTextToIdSuffix) {
  const id = context.getFilename() ? generateId(context, idPrefix) : "";
  return id + "." + extractTextToIdSuffix(text);
}

function defaultLiteralSubstitution(node, context, fixer, idPrefix,extractTextToIdSuffix) {
  const sourceCode = context.getSourceCode();
  const text = sourceCode.getText(node);
  let id = defaultLiteralIdGenerator(context, text, idPrefix,extractTextToIdSuffix);
...
function defaultAttributeSubstitution(node, context, fixer, idPrefix,extractTextToIdSuffix) {
  const sourceCode = context.getSourceCode();
  const text = sourceCode.getText(node);
  const id = defaultAttributeIdGenerator(context, node, text, idPrefix,extractTextToIdSuffix);
...
module.exports = {
  ...
  create(context) {
    const defaults = {
      ...
      extractTextToIdSuffixFn: defaultExtractTextToIdSuffix,
      idPrefix: "",
    ...
    if (typeof config.extractTextToIdSuffixFn === "string") {
      config.extractTextToIdSuffixFn = eval("(" + config.extractTextToIdSuffixFn + ")");
    }
      ...
      report(context, messages[messageId], messageId, {
        ...
        fix: function (fixer) {
          if (messageId === "literalNotInJSXExpression") {
            return config.literalSubstitutionFn(node, context, fixer, config.idPrefix, config.extractTextToIdSuffixFn);
          }
          if (messageId === "noStringsInAttributes") {
            return config.attributeSubstitutionFn(node, context, fixer, config.idPrefix, config.extractTextToIdSuffixFn);
          }
        },
      });
    }
