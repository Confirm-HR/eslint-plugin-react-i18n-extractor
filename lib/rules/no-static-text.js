...
function defaultLiteralSubstitution(node, context, fixer) {
...
function defaultAttributeSubstitution(node, context, fixer) {
  const sourceCode = context.getSourceCode();
  const text = sourceCode.getText(node);
  let id = context.getFilename() ? generateId(context, "app.", text) : "";
      ...
      report(context, messages[messageId], messageId, {
        ...
        fix: function (fixer) {
          if (messageId === "literalNotInJSXExpression") {
            return config.literalSubstitutionFn(node, context, fixer);
          }
          if (messageId === "noStringsInAttributes") {
            return config.attributeSubstitutionFn(node, context, fixer);
          }
        },
      });
    }
